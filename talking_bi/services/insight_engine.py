...
def generate_insights(
    ...
    if dim_col and dim_col in df.columns:
        grouped = _group_sum(df, kpi_col, dim_col)
        if grouped is not None and len(grouped) >= 1:
            top = grouped.idxmax()
            insights.append(f"{kpi_label} is highest in {top}.")

    # 2) Lowest performer
    if grouped is not None and len(grouped) >= 1 and len(insights) < MAX_INSIGHTS:
        low = grouped.idxmin()
        insights.append(f"{kpi_label} is lowest in {low}.")
    ...
    if grouped is not None and len(grouped) >= 1 and len(insights) < MAX_INSIGHTS:
        ...
        if total != 0:
            ...
            insights.append(f"{top} contributes {_format_pct(contrib)} of total {kpi_label.lower()}.")
    ...
    if len(insights) < MAX_INSIGHTS:
        ...
        if time_col and time_col in df.columns:
            ...
            if trend:
                ...
                insights.append(
                    f"{kpi_label} {direction} by {_format_pct(abs(pct))} in the latest period ({latest_str})."
                )
    ...
    if len(insights) < MAX_INSIGHTS:
        ...
        if spike:
            ...
            insights.append(f"Unusual spike detected in {label} for {kpi_label.lower()}.")
