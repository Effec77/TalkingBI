...
async def query_endpoint(session_id: str, payload: QueryPayload):
    ...
    print(f"[API:query] Parsed intent: {intent['intent']}")

    try:
        # Generate dashboard plan (same as /run)
        ...
        # Build response (same as /run + session info)
        response = {
            "session_id": result_state["session_id"],
            "run_id": result_state["run_id"],
            "query": user_query,
            "intent": intent,  # Phase 6B: include parsed intent
